...
void TrafficLight::waitForGreen()
{
  ...
  while(true)
  {
    TrafficLightPhase phase = _messageQueues.receive();
  ...
  while(true)
  {
    ...
    if(timeSinceLastUpdate >= cycleDuration)
    {
      ...
      _currentPhase = (_currentPhase == TrafficLightPhase::red)
        ...
        // Send update via message queue
        _messageQueues.send(std::move(_currentPhase));

        // Reset timer and new cycle duration
